...
/*
 * Pruebas de INTEGRACIÓN del módulo Perfil (usa el modelo Usuarios).
 *
 * Secciones:
 * - Dobles: `StatementDoublePerfilF`, `PDODoublePerfilSF` (seguridad) y `PDODoublePerfilPF` (inventario) simulando consultas, inserts, updates y transacciones.
...
// ======================================
// Dobles de prueba (PDOStatement/Perfil Feature)
// ======================================
class StatementDoublePerfilF
{
    private string $sql;
    private array $rows;
    private ?array $row;
...
// ======================
class PDODoublePerfilSF extends PDO
{
    ...
    public function prepare($statement, array $options = [])
    {
        ...
        if (stripos($sql, 'INSERT INTO tbl_usuarios (username, password, id_rol') !== false) {
            return new StatementDoublePerfilF($sql);
        }
        // UPDATE usuario
        ...
        if (stripos($sql, 'SELECT COUNT(*) FROM tbl_usuarios WHERE username') !== false) {
            return new StatementDoublePerfilF($sql, ['scalar' => 1]);
        }
        if (stripos($sql, 'SELECT COUNT(*) FROM tbl_usuarios WHERE cedula') !== false) {
            return new StatementDoublePerfilF($sql, ['scalar' => 0]);
        }
        ...
        if (stripos($sql, "SHOW COLUMNS FROM tbl_usuarios LIKE 'estatus'") !== false) {
            return new StatementDoublePerfilF($sql, ['rows' => [['Field' => 'estatus']]]);
        }
...
// ======================
class PDODoublePerfilPF extends PDO
{
    public function __construct() {}
    public function setAttribute($attribute, $value) { return true; }
...
// ================================
final class PerfilFeatureTest extends TestCase
{
    ...
    private function nuevoPerfilConPDOStub(): Usuarios
    {
        ...
        $sec = new PDODoublePerfilSF();
        $inv = new PDODoublePerfilPF();
    ...
    // PRF-FEAT-002: Modificar usuario
    ...
    // PRF-FEAT-004: Obtener último usuario
    ...
    // PRF-FEAT-005: Obtener usuario por id
    ...
    // PRF-FEAT-009: Actualizar perfil (update dinámico)
    ...
    // PRF-FEAT-010: Listar usuarios por estatus
