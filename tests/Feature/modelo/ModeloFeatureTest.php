...
/*
 * Pruebas de INTEGRACIÓN del módulo de Modelos.
 *
 * Secciones:
 * - Dobles de prueba: `StatementDoubleModeloF` y `PDODoubleModeloF` (simulan PDO/Statement)
 * - Escenarios: registrar, existe por nombre, obtener último, por ID, combo de marcas,
 *   modificar, eliminar (con/sin productos asociados), obtener modelo con marca, listar modelos.
 */

// ======================================
// Dobles de prueba (PDOStatement/Modelo)
// ======================================
class StatementDoubleModeloF
{
    private string $sql;
    private array $rows;
    private ?array $row;
    ...
    {
        // Validaciones mínimas para INSERT/UPDATE
...
// Doble de prueba (PDO)
// ======================
class PDODoubleModeloF extends PDO
{
    ...
    public function prepare($statement, array $options = [])
    {
        ...
        if (stripos($sql, 'SELECT COUNT(*) FROM tbl_modelos WHERE nombre_modelo') !== false) {
            return new StatementDoubleModeloF($sql, [
    ...
    public function query($query, $fetchMode = null, ...$fetchModeArgs)
    {
        ...
        if (stripos($query, 'SELECT id_marca, nombre_marca FROM tbl_marcas') !== false) {
            return new StatementDoubleModeloF($query, [
                'rows' => [
                    ['id_marca' => 1, 'nombre_marca' => 'MarcaA'],
                    ['id_marca' => 2, 'nombre_marca' => 'MarcaB'],
                ],
...
final class ModeloFeatureTest extends TestCase
{
    ...
    // MOD-FEAT-001: Registrar modelo
    public function testRegistrarModeloIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
        $m->setnombre_modelo('NuevoModelo');
        $m->setid_marca(10);
        $this->assertTrue($m->registrarModelo());
    }

    // MOD-FEAT-002: Existe nombre modelo (true)
    public function testExisteNombreModeloIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
        $this->assertTrue($m->existeNombreModelo('Existente'));
    }

    // MOD-FEAT-003: Obtener último modelo
    public function testObtenerUltimoModeloIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
    ...
    // MOD-FEAT-004: Obtener modelo por id
    public function testObtenerModeloPorIdIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
    ...
    // MOD-FEAT-005: Listar marcas (combo)
    public function testGetMarcasComboIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
        $lista = $m->getmarcas();
    ...
    // MOD-FEAT-006: Modificar modelo
    public function testModificarModeloIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
        $m->setnombre_modelo('Editado');
        $m->setid_marca(9);
        $this->assertTrue($m->modificarModelo(7));
    }

    // MOD-FEAT-007: Eliminar modelo con productos asociados (error)
    public function testEliminarModeloConProductosAsociadosIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
        PDODoubleModeloF::$productosAsociados = true;
    ...
    // MOD-FEAT-008: Eliminar modelo sin productos asociados (success)
    public function testEliminarModeloSinProductosAsociadosIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
        PDODoubleModeloF::$productosAsociados = false;
        $res = $m->eliminarModelo(7);
        $this->assertIsArray($res);
        $this->assertSame('success', $res['status']);
    }

    // MOD-FEAT-009: Obtener modelo con marca por id (JOIN)
    public function testObtenerModeloConMarcaPorIdIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
    ...
    // MOD-FEAT-010: Listar modelos (JOIN)
    public function testGetModelosListadoIntegracion(): void
    {
        $m = $this->nuevoModeloConPDOStub();
        $lista = $m->getModelos();
        $this->assertIsArray($lista);
