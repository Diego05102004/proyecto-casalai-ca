$(document).ready(function () {
    ...
    function anularFilaDespacho(idDespacho) {
        const tabla = $('#tablaConsultas').DataTable();
        const fila = $(`#tablaConsultas tbody tr[data-id="${idDespacho}"]`).addClass('anulada');
    ...
    $(document).on('click', '.btn-anular', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var idDespacho = $(this).data('id-despacho');

                var datos = new FormData();
                datos.append('accion', 'anular');
                datos.append('id_despachos', idDespacho);
                ...
                $.ajax({
                    ...
                    success: function (respuesta) {
                        if (respuesta.status === 'success') {
                            Swal.fire(
                                'Anulado!',
                                'El despacho ha sido anulado.',
                                'success'
                            );
                            anularFilaDespacho(idDespacho);
                        } else {
                            Swal.fire('Error', respuesta.message || 'Error al anular el despacho', 'error');
                        }
                    },
                    error: function () {
                        Swal.fire('Error', 'Error en la solicitud AJAX', 'error');
